  ...
  return (
    <div className="min-h-screen bg-luxury-cream dark:bg-midnight-base text-luxury-charcoal dark:text-luxury-cream transition-colors duration-500 pb-32">
      ...
      <div className="fixed top-6 right-6 z-50 flex gap-2">
         <button onClick={() => setIsDarkMode(!isDarkMode)} className="bg-white dark:bg-midnight-surface p-4 rounded-full shadow-lg border border-luxury-gold/10 text-luxury-gold hover:scale-110 transition-transform">
      ...
      {activeTab === 'dashboard' && (
        <div className="bg-white dark:bg-midnight-surface p-10 rounded-b-[60px] shadow-2xl space-y-10 animate-in slide-in-from-top duration-700 relative overflow-hidden">
          ...
          <div className="flex justify-between items-start relative z-10">
            <div className="space-y-1"><div className="flex items-center gap-2 text-luxury-gold font-black uppercase text-[10px] tracking-[0.3em]"><Award size={16}/> Niv. {profile.level} • {profile.xp} XP</div><h1 className="text-3xl font-serif font-black">Votre Palais</h1></div>
            <button onClick={() => { signOut(); setSession(null); }} className="bg-luxury-cream dark:bg-black p-3 rounded-2xl text-luxury-bordeaux hover:bg-luxury-bordeaux hover:text-white transition-all shadow-sm"><LogOut size={20}/></button>
          </div>
      ...
      <main className="p-6 max-w-lg mx-auto space-y-8 mt-4">
          ...
          <div className="space-y-10 animate-in fade-in">
             ...
             <div className="space-y-6">
                {WORKOUT_DATABASE.filter(w => w.location === profile.workoutLocation).map(w => (
                  <div key={w.id} onClick={() => setActiveWorkout(w)} className="bg-white dark:bg-midnight-surface p-8 rounded-[56px] shadow-2xl border border-luxury-gold/5 flex justify-between items-center group cursor-pointer hover:border-luxury-gold transition-all relative overflow-hidden">
          ...
          <div className="p-8 space-y-8 animate-in fade-in max-w-lg mx-auto">
             ...
             <div className="space-y-6">
               {dailyPlan.map(recipe => (
                 <div key={recipe.id} onClick={() => { setSelectedRecipe(recipe); setPortions(1); }} className="bg-white dark:bg-midnight-surface p-7 rounded-[56px] shadow-2xl border border-luxury-gold/5 flex gap-6 items-center group cursor-pointer hover:scale-[1.03] transition-all relative overflow-hidden">
          ...
          <div className="space-y-10 animate-in fade-in">
             ...
             {Object.entries(shoppingList).map(([cat, items]) => (
               <div key={cat} className="space-y-4">
                  ...
                  <div className="bg-white dark:bg-midnight-surface rounded-[48px] shadow-xl overflow-hidden border border-luxury-gold/5">
        ...
        {activeTab === 'progress' && (
          <div className="space-y-10 animate-in fade-in">
             <div className="bg-white dark:bg-midnight-surface p-10 rounded-[56px] shadow-2xl border border-luxury-gold/5">
                <div className="flex justify-between items-center mb-10"><h3 className="text-luxury-gold font-black uppercase text-[10px] tracking-[0.4em] flex items-center gap-2"><TrendingUp size={18}/> Courbe Prestige</h3><div className="bg-luxury-cream dark:bg-black px-4 py-1 rounded-full text-[10px] font-black text-luxury-gold shadow-sm">OBJECTIF {profile.targetWeight} KG</div></div>
      ...
      {isConciergeOpen && (
        <div className="fixed inset-x-4 bottom-32 z-[70] bg-white dark:bg-midnight-surface rounded-[48px] shadow-[0_40px_100px_-20px_rgba(0,0,0,0.6)] border border-luxury-gold/20 flex flex-col overflow-hidden animate-in slide-in-from-bottom duration-500" style={{maxHeight: '60vh'}}>
           <div className="bg-luxury-charcoal dark:bg-black p-6 flex justify-between items-center">
              ...
              <button onClick={() => setIsConciergeOpen(false)} className="text-white/50 hover:text-white"><X/></button>
           </div>
           <div className="flex-1 p-6 overflow-y-auto space-y-6 bg-luxury-cream/30 dark:bg-black/30">
              {chatMessages.map((m, i) => (
                <div key={i} className={cn("flex", m.role === 'user' ? "justify-end" : "justify-start")}>
                   <div className={cn("max-w-[85%] p-5 rounded-[32px] text-sm leading-relaxed", m.role === 'user' ? "bg-luxury-bordeaux text-white rounded-br-none" : "bg-white dark:bg-midnight-surface border border-luxury-gold/10 rounded-bl-none shadow-sm")}>
                      {m.text}
                   </div>
                </div>
              ))}
              {isChatLoading && <div className="flex justify-start"><div className="bg-white dark:bg-midnight-surface p-5 rounded-[32px] rounded-bl-none shadow-sm"><Loader2 className="animate-spin text-luxury-gold" size={20}/></div></div>}
           </div>
           <div className="p-4 bg-white dark:bg-midnight-surface border-t border-luxury-gold/10 flex gap-3">
              <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyPress={e => e.key === 'Enter' && askCoach()} placeholder="Posez votre question..." className="flex-1 bg-luxury-cream dark:bg-black rounded-3xl px-6 py-4 outline-none text-sm focus:ring-1 focus:ring-luxury-gold" />
              <button onClick={askCoach} disabled={isChatLoading || !chatInput} className="bg-luxury-gold text-white p-4 rounded-full disabled:opacity-50"><ChevronRight size={24}/></button>
           </div>
        </div>
      )}
      ...
      {selectedRecipe && (
        <div className="fixed inset-0 z-[150] bg-luxury-cream dark:bg-midnight-base overflow-y-auto animate-in slide-in-from-bottom duration-700">
           <div className="p-10 max-w-lg mx-auto space-y-12 pb-48">
              <button onClick={() => {setSelectedRecipe(null); setPortions(1);}} className="bg-white dark:bg-midnight-surface p-5 rounded-[24px] shadow-xl text-luxury-bordeaux hover:rotate-90 transition-all shadow-luxury-gold/10"><X size={24}/></button>
              ...
              <div className="bg-white dark:bg-midnight-surface p-6 rounded-[40px] shadow-sm border border-luxury-gold/10 flex items-center justify-between px-10">
                 ...
                 <div className="flex items-center gap-6">
                    <button onClick={() => setPortions(p => Math.max(1, p-1))} className="p-3 bg-luxury-cream dark:bg-black rounded-full text-luxury-bordeaux"><Minus size={20}/></button>
                    <span className="text-4xl font-black w-10 text-center">{portions}</span>
                    <button onClick={() => setPortions(p => p+1)} className="p-3 bg-luxury-cream dark:bg-black rounded-full text-luxury-gold"><Plus size={20}/></button>
                 </div>
              </div>
                 ...
                 <div className="space-y-5">
                    ...
                    {selectedRecipe.ingredients.map((ing, i) => (
                      <div key={i} className="flex justify-between items-center border-b border-luxury-cream dark:border-black pb-5 last:border-0 last:pb-0 group">
                         ...
                         <span className="text-sm font-black text-luxury-gold bg-luxury-cream dark:bg-black px-4 py-2 rounded-xl">{ing.quantity * portions} {ing.unit}</span>
                      </div>
                    ))}
                 </div>
              </div>
      ...
      {activeWorkout && (
        <div className="fixed inset-0 z-[150] bg-luxury-cream dark:bg-midnight-base overflow-y-auto animate-in slide-in-from-bottom duration-700">
           <div className="p-10 max-w-lg mx-auto space-y-12 pb-48">
              <button onClick={() => setActiveWorkout(null)} className="bg-white dark:bg-midnight-surface p-5 rounded-[24px] shadow-xl text-luxury-bordeaux hover:rotate-90 transition-all"><X size={24}/></button>
                 ...
                 {activeWorkout.exercises.map((ex, i) => (
                   <div key={i} className="bg-white dark:bg-midnight-surface p-8 rounded-[48px] shadow-2xl border border-luxury-gold/5 space-y-6">
                      <div className="flex justify-between items-start">
                         ...
                         <div className="bg-luxury-cream dark:bg-black p-4 rounded-3xl text-luxury-bordeaux dark:text-luxury-gold font-black">{ex.sets} x {ex.reps}</div>
                      </div>
                      <div className="space-y-3">
                         <span className="text-[10px] font-black text-luxury-gold uppercase tracking-widest pl-2">Charge utilisée (KG)</span>
                         <input type="text" placeholder="Entrez votre poids..." className="w-full p-5 bg-luxury-cream dark:bg-black rounded-2xl border-none outline-none ring-1 ring-luxury-gold/10 focus:ring-2 focus:ring-luxury-gold font-black bg-transparent" />
                      </div>
                   </div>
                 ))}
              </div>
