...
const WORKOUT_DATABASE: Workout[] = [
  ...
  { id: 'm1', title: 'Séance A : Poussée', focus: 'Pecs, Épaules, Triceps', location: 'GYM', duration: 60, caloriesBurned: 350, exercises: [
    ...
    { id: 'e3', name: 'Écartés Poulie', sets: 3, reps: '12' },
    { id: 'e4', name: 'Extensions Triceps', sets: 4, reps: '12' },
    { id: 'e5', name: 'Dips', sets: 3, reps: 'Échec' }
  ]},
  { id: 'm2', title: 'Séance B : Tirage', focus: 'Dos, Biceps', location: 'GYM', duration: 60, caloriesBurned: 350, exercises: [
    { id: 'e6', name: 'Tractions', sets: 4, reps: '8-10' },
    { id: 'e7', name: 'Rowing Barre', sets: 4, reps: '10' },
    { id: 'e8', name: 'Oiseau Poulie', sets: 3, reps: '15' },
    { id: 'e9', name: 'Curl Biceps', sets: 3, reps: '12' }
  ]},
  { id: 'm3', title: 'Séance C : Jambes', focus: 'Force & Volume', location: 'GYM', duration: 70, caloriesBurned: 450, exercises: [
    { id: 'e10', name: 'Squat Barre', sets: 4, reps: '8' },
    { id: 'e11', name: 'Presse à cuisses', sets: 3, reps: '12' },
    { id: 'e12', name: 'Leg Extension', sets: 3, reps: '15' }
  ]},
  // PERTE
  { id: 'p1', title: 'Full Body Brûle-Gras', focus: 'Circuit Cardio', location: 'HOME', duration: 30, caloriesBurned: 400, exercises: [
    ...
    { id: 'e14', name: 'Pompes', sets: 4, reps: '15' },
    { id: 'e15', name: 'Mountain Climbers', sets: 4, reps: '45s' }
  ]},
  { id: 'p2', title: 'HIIT & Abdos', focus: 'Séchage intense', location: 'HOME', duration: 25, caloriesBurned: 300, exercises: [
    { id: 'e16', name: 'Burpees', sets: 3, reps: '12' },
    { id: 'e17', name: 'Planche', sets: 3, reps: '1 min' },
    { id: 'e18', name: 'Russian Twist', sets: 3, reps: '30' }
  ]}
];
...
function App() {
  ...
  const handleGenerate = async (prof = profile) => {
    ...
    const filtered = INTERNAL_RECIPES.filter(r => 
      r.tags.includes(prof.goal) && r.tags.includes(prof.budget)
    );
    if (filtered.length > 0) {
      setDailyPlan(filtered.slice(0, 3));
    } else {
      ...
      <div className="min-h-screen bg-luxury-cream p-8 flex flex-col justify-center items-center space-y-12 animate-in fade-in duration-1000">
        ...
        <div className="w-full max-w-sm space-y-6 bg-white p-10 rounded-[60px] shadow-2xl border border-luxury-gold/10 relative overflow-hidden">
           ...
           <div className="space-y-6">
              ...
              <button onClick={() => setAuthMode(authMode === 'signin' ? 'signup' : 'signin')} className="w-full text-xs font-black text-luxury-gold uppercase tracking-widest underline">{authMode === 'signin' ? "Rejoindre le Club" : "Déjà Membre ?"}</button>
              <button onClick={signInWithGoogle} className="w-full bg-white border border-gray-100 text-gray-600 font-bold py-4 rounded-3xl flex items-center justify-center gap-3 shadow-sm hover:bg-gray-50 transition-all mt-4"><img src="https://www.google.com/favicon.ico" className="w-5 h-5" alt="Google" /> Google Login</button>
           </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-luxury-cream pb-32">
      ...
      {activeTab === 'dashboard' && (
        <div className="bg-white p-10 rounded-b-[60px] shadow-2xl space-y-10 animate-in slide-in-from-top duration-700 relative overflow-hidden">
          ...
          <div onClick={() => fileInputRef.current?.click()} className="bg-luxury-charcoal p-7 rounded-[40px] text-white flex items-center justify-between shadow-2xl group cursor-pointer hover:bg-black transition-all">
            <div className="flex items-center gap-5 relative z-10"><div className="bg-luxury-gold p-4 rounded-2xl shadow-inner"><Camera size={28} className="text-luxury-charcoal" /></div><div><p className="text-lg font-black leading-none mb-1">L'Œil Vision IA</p><p className="text-[10px] opacity-60 uppercase font-black tracking-widest tracking-[0.2em]">Analyser mon plat gastronomique</p></div></div>
      ...
      <main className="p-6 max-w-lg mx-auto space-y-8 mt-4">
        {activeTab === 'studio' && (
          <div className="space-y-10 animate-in fade-in">
             ...
             <div className="space-y-6">
                {WORKOUT_DATABASE.filter(w => w.location === profile.workoutLocation && (profile.goal === 'MUSCLE_GAIN' ? w.id.startsWith('m') : w.id.startsWith('p'))).map(w => (
                  <div key={w.id} onClick={() => setActiveWorkout(w)} className="bg-white p-8 rounded-[56px] shadow-2xl border border-luxury-gold/5 flex justify-between items-center group cursor-pointer hover:border-luxury-gold transition-all relative overflow-hidden">
                     <div className="flex items-center gap-6 relative z-10">
                        <div className="w-20 h-20 bg-luxury-cream rounded-[32px] flex items-center justify-center text-luxury-bordeaux shadow-inner border border-luxury-gold/5 group-hover:scale-110 transition-transform"><Target size={32}/></div>
                     ...
                     <div className="text-right pr-2 relative z-10"><span className="text-xl font-black text-luxury-gold">+{w.caloriesBurned}</span><p className="text-[8px] font-black uppercase text-gray-300 tracking-tighter">Kcal Bonus</p></div>
                  </div>
                ))}
             </div>
          </div>
               ...
               <div key={cat} className="space-y-4">
                  ...
                  <div className="bg-white rounded-[48px] shadow-xl overflow-hidden border border-luxury-gold/5">
                     {(items as any[]).map((ing, i) => {
                        ...
                        return (
                           <div key={i} onClick={() => setCheckedItems(prev => isChecked ? prev.filter(it => it !== ing.name) : [...prev, ing.name])} className={cn("flex justify-between items-center p-7 border-b border-luxury-cream last:border-0 hover:bg-luxury-cream/10 transition-all group cursor-pointer", isChecked && "bg-luxury-cream/40 opacity-50")}>
                              <div className="flex items-center gap-5">
                                 <div className={cn("w-7 h-7 border-2 border-luxury-gold/20 rounded-xl group-hover:border-luxury-gold transition-all flex items-center justify-center", isChecked && "bg-luxury-gold border-luxury-gold")}>{isChecked && <CheckCircle2 size={16} className="text-white" />}</div>
        ...
        {activeTab === 'progress' && (
          <div className="space-y-10 animate-in fade-in">
             <div className="bg-white p-10 rounded-[56px] shadow-2xl border border-luxury-gold/5">
                <div className="flex justify-between items-center mb-10"><h3 className="text-luxury-gold font-black uppercase text-[10px] tracking-[0.4em] flex items-center gap-2"><TrendingUp size={18}/> Courbe Prestige</h3><div className="bg-luxury-cream px-4 py-1 rounded-full text-[10px] font-black text-luxury-gold shadow-sm">OBJECTIF {profile.targetWeight} KG</div></div>
             ...
             <div className="bg-luxury-charcoal p-10 rounded-[60px] shadow-2xl text-white space-y-8 relative overflow-hidden">
      ...
      <nav className="fixed bottom-10 left-8 right-8 bg-luxury-charcoal/95 backdrop-blur-3xl p-6 rounded-[56px] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.6)] flex justify-around items-center z-50 border border-white/5">
        {[
          ...
          <button key={item.id} onClick={() => setActiveTab(item.id as any)} className={cn("flex flex-col items-center gap-2 transition-all duration-500", activeTab === item.id ? "text-luxury-gold scale-125 -translate-y-1" : "text-luxury-gold/20 hover:text-luxury-gold/40 group")}>
           ...
           <div className="p-10 max-w-lg mx-auto space-y-12 pb-48">
              ...
              <div className="bg-white p-10 rounded-[56px] shadow-xl border border-luxury-gold/5 space-y-8">
                 ...
                 <div className="space-y-5">
                    {selectedRecipe.ingredients.length === 0 && <div className="text-center italic text-gray-300 uppercase text-[10px] tracking-widest py-10">Ingrédients exclusifs en cours de préparation...</div>}
           ...
           <div className="p-10 max-w-lg mx-auto space-y-12 pb-48">
              ...
              <div className="space-y-6">
                 {activeWorkout.exercises.map((ex, i) => (
                   <div key={i} className="bg-white p-8 rounded-[48px] shadow-2xl border border-luxury-gold/5 space-y-6">
